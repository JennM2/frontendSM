...
import useStyles from "./admins.style";
...
const Admins = () => {
  ...
  const loadData = () => {
    Axios.get(`${process.env.REACT_APP_SERVER_HOST}/api/admin`)
      .then((res) => {
        const dataArray = res.data.map((admin,index) => [
          index + 1,
          admin.user,
          admin.paterno,
          admin.materno,
  ...
  const handleSave = () => {
    ...
    Axios.post(`${process.env.REACT_APP_SERVER_HOST}/api/admin`, newRow)
      .then((res) => {
        loadData();
  ...
  const handleFinalDeletion = () => {
    Axios.delete(`${process.env.REACT_APP_SERVER_HOST}/api/admin/disable/${idSpecific}`)
      .then(() => {
  ...
  const handleEditSave = () => {
    if (editingRow) {
      ...
      Axios.put(
        `${process.env.REACT_APP_SERVER_HOST}/api/admin/${editedId}`,
        editedRowData
      )
        .then((res) => {
          loadData();
    ...
    return (
      <div className={adminClasses.content}>
        <div className={classes.title}>
          <p className={adminClasses.text}>ADMINISTRADORES</p>
