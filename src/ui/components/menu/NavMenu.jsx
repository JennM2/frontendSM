...
const NavMenu = ({
  ...
  const handleOptionClick = (option) => {
    if (option.path === "/login") {
      localStorage.removeItem("credentialSM");
    } else {
            ...
            >
              <div
                className={`${classes.info} ${
                  selectedOption === option.name && classes.selectedInfo
                }`}
              >
                <img
                  className={classNameIcon || classes.icon}
                  src={option.image}
                  alt={option.name}
                />
                {option.name}
              ...
              {option.suboptions.map((suboption) => (
                <li className={classes.listSub} key={suboption.name}>
                  <Link to={suboption.path} className={classes.link}>
                    <button
                      className={classes.buttonSuboption}
                      onClick={() => setSelectedSubOption(suboption.nameLabel)}
                    >
                      <div
                        className={`${classes.infoSuboption} ${
                          selectedSubOption === suboption.nameLabel &&
                          classes.selectedInfoSub
                        }`}
                      >
                        <img
